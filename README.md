...
`IStoreAPI` - интерфейс для реализации методов работы с сервером - получения и отправки данных.
...
`setProductInBasket(product: IProduct): IProduct` - добавление товара, который был получен в параметре в массив корзины.
...
Методы класса:
`setBuyerData(buyer: IBuyer): void` - сохранение данных в модели. Один общий метод или отдельные методы для каждого поля.
`getBuyerData(): IBuyer` - получение всех данных покупателя.
`clearBuyerData(): void` - очистка данных покупателя.
`validateBuyerData(buyer: IBuyer): void` - валидация данных.
...
Конструктор:
`constructor(cdn: string, baseUrl: string, options?: RequestInit)` - в конструктор передается строка подключения к cdn, базовый адрес сервера и опциональный объект с заголовками запросов.
